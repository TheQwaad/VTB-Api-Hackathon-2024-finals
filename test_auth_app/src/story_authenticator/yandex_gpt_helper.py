...
class YandexGptHelper:
    ...
    def make_request(self, messages=None, stream=False, temperature=0.5, max_token='100'):
        if messages is None:
        ...
        if response.status_code == 200:
            ...
            raise requests.RequestException(
                f"Unexpected status: {response.status_code}, message: {response.text}"
            )

    def make_message_request(self, text, role='system', stream=False, temperature=0.5, max_token='100'):
        response_data = self.make_request(messages=[{'role': role, 'text': text}], stream=stream,
